...
**PWA packaging and sync triggers implemented, August 10, 2026** (Phase 5 of the design): icons generated by new `scripts/generate-pwa-icons.mjs` from the same master `src-tauri/icons/icon.png` `tauri icon` already produces (`sharp`, same pattern as `scripts/optimize-stock-backgrounds.mjs`) — plain 192/512px "any" icons, 192/512px "maskable" icons (artwork padded to the safe ~70% center on a canvas in the app's own background color, since an OS may crop these to a circle/rounded-square), and a flattened 180px `apple-touch-icon.png` (iOS renders transparency as black, so this one is never left transparent). `public/manifest.webmanifest` uses relative paths (`start_url`/`scope`: `"."`, icon `src`s with no leading slash) so it resolves correctly regardless of whether the app is deployed at the root or nested under the GitHub Pages demo's `/WorshipStudio/app/` path. `index.html` links it plus `apple-mobile-web-app-*` meta tags — iOS ignores the manifest's own `display: "standalone"`, these are what actually make "Add to Home Screen" launch without Safari's chrome.

Service worker via the new `vite-plugin-pwa` dependency (`registerType: 'autoUpdate'`, `manifest: false` since `index.html` already links the static one directly). Scoped strictly to the app shell (`globPatterns`: js/css/html/icons/fonts) — deliberately no `runtimeCaching` entries for any Dropbox/Graph API host, since `cloudSync.ts`'s own cursor/rev state is the real source of truth for what's synced, not an HTTP cache, and a service worker silently serving a stale cached API response would be actively wrong here. One real build failure surfaced and fixed: Workbox's default 2MiB precache limit rejected the bundled ~4.4MB KJV text dataset chunk — excluded via `globIgnores` rather than raising the limit, since that chunk is already lazily fetched only once scripture lookup is actually used (unchanged from before this plugin existed); precaching it during SW install would have slowed down the initial "install this app" moment for no benefit.

Install prompt: new `src/composables/usePwaInstall.ts` captures `beforeinstallprompt` (Android/desktop Chrome) and separately flags iOS Safari, which has no programmatic install API at all — wired into `BootGate.vue`'s chooser screen as an "Install as an App" button where available, or a persistent "tap Share → Add to Home Screen" tip on iOS.

Sync triggers: new `src/composables/useTabletSync.ts`, instantiated unconditionally in `App.vue` (a no-op for every non-tablet adapter kind) — syncs once on mount, on every tab-visible transition, on window focus, and on a 5-minute timer that only runs while the tab is actually visible. The debounced push-only pass after each local save described in the original plan was deliberately not added: wiring it in would mean threading a trigger through every individual store's save/delete action, and `cloudSync.ts`'s `pull()` already protects any dirty (unpushed) path from being clobbered by a concurrent pull, so a save is never at risk between now and the next visibility/focus/interval tick — just not pushed quite as instantly. The existing manual "Sync Now" button already covers "I want this synced right now."

New tests: `usePwaInstall.spec.ts` and `useTabletSync.spec.ts` (mounted through a minimal host component via `@vue/test-utils`, since both rely on `onMounted`/`onUnmounted`). Full validation: `type-check`, `lint`, the full `test:unit` suite (510 passed), a production `build` (confirmed `dist/sw.js`, `dist/manifest.webmanifest`, and `dist/pwa-icons/` all generated correctly, precaching 184 entries), and `cargo build` (unaffected — this phase touched no Rust code).

**Still not started**: real-account verification of OneDrive push/pull/conflict handling and the QR/link add-device flow scanned from a genuinely second device; the install prompt/service worker/manifest have not yet been exercised on real iOS/Android hardware; and the real-device hardening pass (OPFS quota/eviction behavior on iOS/Android, install flow, OAuth redirect round-trip, backgrounded-tab gaps) — Phase 6 of the design.
