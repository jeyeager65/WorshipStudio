...
<template>
  ...
  >
    <template #action>
      <v-btn variant="flat" color="primary" prepend-icon="mdi-plus" @click="openNewExternalAppProfile">
        Add Profile
      </v-btn>
    </template>
      ...
      <v-card-text>
        ...
        <template v-if="editingProfile.launchMode === 'launch-automatically'">
          ...
          <div class="param-preview mb-4">
            Will run:
            {{
              previewExternalAppCommand(editingProfile.executablePath, editingProfile.parameterFormat)
            }}
          </div>
        </template>
